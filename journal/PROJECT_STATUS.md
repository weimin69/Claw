...
最近一次学习中，继续增强了 `send_chat_request()` 的 async HTTP 失败路径。`chat` 命令现在使用 `reqwest::Client::builder()` 创建 client，并为请求设置 60 秒 timeout；`.send().await`、`response.text().await` 和 client 构造失败都添加了明确的错误上下文。学习中还重点讨论了 `Context` 和 `bail!` 的区别、`response.text().await` 会消费 `response` 的所有权、为什么需要先保存 `status`，以及为什么默认不直接打印 provider 错误 body。当前 `cargo fmt --check`、`cargo check`、`cargo test` 均通过，测试数量为 17 个。
...
下一步将 `chat` 中学到的 HTTP 失败路径设计迁移到 `fetch` 命令：

- Review `fetch` 当前的网络错误、HTTP status 错误和 body 读取错误边界
- 讨论 `fetch` 是否也应该使用 `reqwest::Client::builder()` 和 timeout
- 讨论 `fetch` 是否需要 `.context(...)` 补充错误发生阶段
- 继续保持学习重点在 async HTTP 失败路径和用户输出边界

优先学习目标仍然是错误上下文、async HTTP 失败路径和 CLI 用户体验，而不是继续扩展 Agent 功能。
